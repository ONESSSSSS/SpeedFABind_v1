...
# import cutlass_bindings  # Assuming CUTLASS bindings are available

class MC_E_GCL(nn.Module):
    """
    Multi-Channel E(n) Equivariant Convolutional Layer with CUTLASS optimization
    """
    ...
    def __init__(self, args, input_nf, output_nf, hidden_nf, n_channel, edges_in_d=0, act_fn=nn.SiLU(),
                 ...
                 dropout=0.1, coord_change_maximum=10, use_amp=True):
        ...
        # Create optimized MLP modules
        self.edge_mlp = self._create_optimized_mlp([input_edge + n_channel**2 + edges_in_d, hidden_nf, hidden_nf], act_fn)
        self.node_mlp = self._create_optimized_mlp([hidden_nf + input_nf, hidden_nf, output_nf], act_fn)

        # Create coord MLP with special initialization
        coord_layers = []
        coord_layers.append(self._create_linear_layer(hidden_nf, hidden_nf))
        coord_layers.append(act_fn)
        
        layer = nn.Linear(hidden_nf, n_channel, bias=False)
        torch.nn.init.xavier_uniform_(layer.weight, gain=0.001)
        # Convert layer weights to FP16 for optimization if using AMP
        if self.use_amp:
        ...
        if self.attention:
            self.att_mlp = self._create_optimized_mlp([hidden_nf, 1], nn.Sigmoid())
            
        self.coord_change_maximum = coord_change_maximum
        
        # Initialize CUTLASS gemm handlers if available
        self._init_cutlass_gemm()

    def _init_cutlass_gemm(self):
        """Initialize CUTLASS GEMM handlers for optimized matrix multiplication"""
        try:
            # This is a placeholder for actual CUTLASS initialization
            # In a real implementation, you would create CUTLASS GEMM plan objects
            # for each matrix multiplication size in your network
            self.cutlass_plans = {}
        except:
            print("CUTLASS initialization failed, falling back to PyTorch implementation")
            self.cutlass_plans = None

    def _create_linear_layer(self, in_features, out_features, bias=True):
        """Create a linear layer with weight optimization for Tensor Cores"""
        layer = nn.Linear(in_features, out_features, bias=bias)
        # Convert weights to FP16 if using AMP
        if self.use_amp:
            layer.weight.data = layer.weight.data.half()
    ...
    def _create_optimized_mlp(self, layer_sizes, final_activation=None):
        layers = []
        for i in range(len(layer_sizes) - 1):
            layers.append(self._create_linear_layer(layer_sizes[i], layer_sizes[i+1]))
            if i < len(layer_sizes) - 2:
                # 中间层激活函数：使用类并实例化（如 nn.SiLU()）
                layers.append(nn.SiLU())
            elif final_activation is not None:
                if isinstance(final_activation, type) and issubclass(final_activation, nn.Module):
                    # 若是类，实例化后添加
    ...
    def edge_model(self, source, target, radial, edge_attr):
        """
        Optimized edge model using mixed precision and CUTLASS
        """
        radial = radial.reshape(radial.shape[0], -1)  # [n_edge, n_channel ^ 2]

        with autocast(enabled=self.use_amp):
            if edge_attr is None:
                with torch.profiler.record_function("edge_cat"):
                    out = torch.cat([source, target, radial], dim=1)
            else:
                with torch.profiler.record_function("edge_cat_with_attr"):
                    out = torch.cat([source, target, radial, edge_attr], dim=1)
            
            with torch.profiler.record_function("edge_mlp_forward"):
                out = self.edge_mlp(out)
            
            with torch.profiler.record_function("edge_dropout"):
                out = self.dropout(out)
            
            if self.attention:
                with torch.profiler.record_function("edge_attention"):
    ...
    def coord_model(self, coord, edge_index, coord_diff, edge_feat):
        """
        Optimized coordinate model using mixed precision
        """
        row, col = edge_index
        
        with autocast(enabled=self.use_amp):
            with torch.profiler.record_function("coord_mlp"):
                edge_feat_transform = self.coord_mlp(edge_feat).unsqueeze(-1)
            
            with torch.profiler.record_function("coord_transform"):
                trans = coord_diff * edge_feat_transform  # [n_edge, n_channel, d]
            
            if self.coords_agg == 'sum':
                with torch.profiler.record_function("coord_agg_sum"):
                    agg = unsorted_segment_sum(trans, row, num_segments=coord.size(0))
            elif self.coords_agg == 'mean':
                with torch.profiler.record_function("coord_agg_mean"):
                    agg = unsorted_segment_mean(trans, row, num_segments=coord.size(0))
            else:
                raise Exception(f'Wrong coords_agg parameter: {self.coords_agg}')
                
            with torch.profiler.record_function("coord_update"):
    ...
    def node_model(self, x, edge_index, edge_attr, node_attr):
        """
        Optimized node model using mixed precision and CUTLASS
        """
        row, col = edge_index
        
        with autocast(enabled=self.use_amp):
            with torch.profiler.record_function("node_agg"):
                agg = unsorted_segment_sum(edge_attr, row, num_segments=x.size(0))
                
            with torch.profiler.record_function("node_cat"):
                if node_attr is not None:
                    agg = torch.cat([x, agg, node_attr], dim=1)
                else:
                    agg = torch.cat([x, agg], dim=1)
                    
            with torch.profiler.record_function("node_mlp"):
                out = self.node_mlp(agg)
                
            with torch.profiler.record_function("node_dropout"):
                out = self.dropout(out)
                
            if self.residual:
                with torch.profiler.record_function("node_residual"):
                    out = x + out
    ...
    def forward(self, h, edge_index, coord, edge_attr=None, node_attr=None, batch_id=None):
        ...
        """
        # Use mixed precision for the entire forward pass
        with autocast(enabled=self.use_amp):
            with torch.profiler.record_function("coord2radial"):
                row, col = edge_index
                radial, coord_diff = coord2radial(edge_index, coord, self.args.rm_F_norm, batch_id=batch_id, norm_type=self.args.norm_type)
                
            with torch.profiler.record_function("edge_model"):
                edge_feat = self.edge_model(h[row], h[col], radial, edge_attr)
                
            with torch.profiler.record_function("coord_model"):
                coord = self.coord_model(coord, edge_index, coord_diff, edge_feat)
                
            with torch.profiler.record_function("node_model"):
...
#                 with torch.profiler.record_function("1"):
#                     out = torch.cat([source, target, radial], dim=1)
#             else:
#                 with torch.profiler.record_function("2"):
#                     out = torch.cat([source, target, radial, edge_attr], dim=1)
#             with torch.profiler.record_function("3"):
#                 out = self.edge_mlp(out)
#             with torch.profiler.record_function("4"):
#                 out = self.dropout(out)
#             with torch.profiler.record_function("5"):
#                 if self.attention:
#                     att_val = self.att_mlp(out)
#                     out = out * att_val
#             return out
...
class MC_Att_L(nn.Module):
    ...
    def trio_encoder(self, h, edge_index, coord, pair_embed_batched=None, pair_mask=None, batch_id=None, segment_id=None, reduced_tuple=None, LAS_mask=None, p_p_dist_embed=None, c_c_dist_embed=None):
        ...
        with torch.profiler.record_function("trioencoder"): 
...
class MCAttEGNN(nn.Module):
    ...
    def forward(self, h, x, ctx_edges, att_edges, LAS_edge_list, batched_complex_coord_LAS, 
        ...
        with torch.profiler.record_function("MCAttEGNN"): 
            h = self.linear_in(h)
            h = self.dropout(h)
            x = x.clone()

            ctx_states, ctx_coords, atts = [], [], []
            with torch.profiler.record_function("_modules"): 
                for i in range(0, self.n_layers):
                    h, coord = self._modules[f'gcl_{i}'](h, ctx_edges, x, batch_id=batch_id, edge_attr=ctx_edge_attr)
                    with torch.profiler.record_function("append hx"): 
                        ...
                        ctx_coords.append(x)
                        # attention bias
                    with torch.profiler.record_function("add_attn_pair_bias"):   ##
                        if self.args.add_attn_pair_bias:
                            if self.args.explicit_pair_embed:
                                with torch.profiler.record_function("explicit_pair_embed"):   ##
                                ...
                                with torch.profiler.record_function("no explicit_pair_embed"):
                                    h, coord, att = self._modules[f'att_{i}'](
                                        h, att_edges, x, edge_attr=att_edge_attr, 
                                        segment_id=segment_id, batch_id=batch_id, reduced_tuple=reduced_tuple
                                        )
                        else:
                            with torch.profiler.record_function("no add_attn_pair_bias"):
                                h, coord, att = self._modules[f'att_{i}'](h, att_edges, x, batch_id=batch_id, edge_attr=att_edge_attr)
                    with torch.profiler.record_function("append(att)"): 
                        if self.args.fix_pocket:
                            x[mask] = coord[mask]
                        else:
                            x = coord
                    ...
                    with torch.profiler.record_function("rm_LAS_constrained_optim"):   ##
                        if not self.args.rm_LAS_constrained_optim:
                            ...
                            x.unsqueeze_(1)
            with torch.profiler.record_function("out_layer"): 
                h, coord = self.out_layer(h, ctx_edges, x, batch_id=batch_id, edge_attr=ctx_edge_attr)
                with torch.profiler.record_function("fix_pocket"):
                    if self.args.fix_pocket:
                        x[mask] = coord[mask]
                    else:
                        x = coord
                    ctx_states.append(h)
                    ctx_coords.append(x)
                with torch.profiler.record_function("dense"):
                    if self.dense:
...
class MCnoAttEGNN(nn.Module):
    ...
    def forward(self, h, x, ctx_edges, att_edges, LAS_edge_list, batched_complex_coord_LAS,
                ...
                p_p_dist_embed=None, c_c_dist_embed=None, mask=None,
                ctx_edge_attr=None, att_edge_attr=None, return_attention=False):
        with torch.profiler.record_function("MCnoAttEGNN"): 
            h = self.linear_in(h)
            h = self.dropout(h)
            x = x.clone()
...
class MCnoAttwithCrossAttEGNN(nn.Module):
    ...
    def forward(self, h, x, ctx_edges, att_edges, LAS_edge_list, batched_complex_coord_LAS, 
                ...
                p_p_dist_embed=None, c_c_dist_embed=None, mask=None,
                ctx_edge_attr=None, att_edge_attr=None, return_attention=False):
        with torch.profiler.record_function("MCnoAttwithCrossAttEGNN"): 
            h = self.linear_in(h)
            h = self.dropout(h)
            x = x.clone()
    ...
    with torch.profiler.record_function("2"):
        if not rm_F_norm:
            if norm_type == 'all_sample':
                ...
                with torch.profiler.record_function("23"):
